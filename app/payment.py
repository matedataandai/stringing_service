...
def SquarePaymentUI(amount=10.00, description="Video recording for Court 1 at Tennis Club", currency="AUD"):
    ...
    with st.form("payment_form"):
        ...
        # Replaced disabled number_input with a clean text display to avoid serialization bugs
        st.markdown(f"**Amount:** ${amount:.2f} {currency}")
        st.session_state.create_payment_link_clicked = st.form_submit_button("Create payment link")

    if st.session_state.create_payment_link_clicked:
